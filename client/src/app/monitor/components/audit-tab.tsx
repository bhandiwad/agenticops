...
function actionBadge(action: string) {
  const cls = ACTION_COLORS[action] || 'bg-muted/10 text-muted-foreground';
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="flex items-center gap-1.5">
            ...
            <input
              ...
              className="bg-muted/50 border border-border/50 rounded-md px-2.5 py-1 text-xs text-foreground placeholder-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring w-36 transition-all"
            />
          </div>
          <input
            type="text"
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {data.events.map((evt, i) => (
                    <tr key={evt.id ?? `${evt.action}-${evt.created_at}-${i}`} className="border-b border-border/40 hover:bg-muted/20 transition-colors duration-150">
                      <td className="px-4 py-2.5 text-muted-foreground text-xs whitespace-nowrap" style={{ fontVariantNumeric: 'tabular-nums' }}>
                        <div className="flex items-center gap-1.5">
                      ...
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-foreground text-xs font-medium truncate max-w-[120px]">{evt.user_id}</span>
                        </div>
                      </td>
                      <td className="px-4 py-2.5">{actionBadge(evt.action)}</td>
                      <td className="px-4 py-2.5 text-muted-foreground text-xs">{evt.resource_type}</td>
                      <td className="px-4 py-2.5 text-muted-foreground text-xs font-mono truncate max-w-[140px]">{evt.resource_id || '—'}</td>
                      <td className="px-4 py-2.5 text-muted-foreground text-xs" style={{ fontVariantNumeric: 'tabular-nums' }}>{evt.ip_address || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <div className="flex items-center justify-between mt-4">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="p-1 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted/60 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                  >
