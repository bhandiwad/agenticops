...
function Panel({ title, subtitle, children }: {
  ...
  return (
    <div className="bg-card/60 border border-border/80 rounded-xl p-5">
      <div className="mb-4">
...
function TriggerBadge({ type }: { readonly type: Action['trigger_type'] }) {
  const styles: Record<string, string> = {
    ...
    manual: 'bg-muted/10 text-muted-foreground',
...
function ModeBadge({ mode }: { readonly mode: Action['mode'] }) {
  return mode === 'agent'
    ...
    : <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-muted/10 text-muted-foreground">Read-Only</span>;
}
...
function ActionsListView({ actions, onSelect, onCreate }: {
  ...
  return (
    <div className="space-y-6">
      ...
      {actions.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center border border-border/80 rounded-xl bg-card/60">
          <Workflow className="h-8 w-8 text-muted-foreground mb-3" />
          <p className="text-sm text-muted-foreground">No actions yet</p>
          <p className="text-xs text-muted-foreground mt-1 mb-4">Create your first action to automate SRE workflows</p>
        ...
        <div className="space-y-3">
          {actions.map((action) => (
            <button
              ...
              className="group w-full text-left bg-card/60 border border-border/80 rounded-xl p-5 cursor-pointer hover:border-border/80 hover:bg-muted/40 transition-all duration-200"
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2.5 mb-1">
                    ...
                    {!action.enabled && (
                      <span className="px-1.5 py-0.5 rounded text-[10px] font-medium bg-muted text-muted-foreground">Disabled</span>
                    )}
                  </div>
                  {action.description && (
                  ...
                  <div className="flex items-center gap-3 mt-3">
                    ...
                    {action.last_run_at && (
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs text-muted-foreground">{formatTimeAgo(action.last_run_at)}</span>
                      </div>
                    )}
                    {action.run_count > 0 && (
                      <span className="text-xs text-muted-foreground">{action.run_count} {action.run_count === 1 ? 'run' : 'runs'}</span>
                    )}
                  </div>
                </div>
                <ChevronRight className="h-4 w-4 text-muted-foreground group-hover:text-muted-foreground transition-colors mt-0.5 flex-shrink-0" />
              </div>
            </button>
          ))}
        </div>
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-semibold tracking-tight text-foreground">{action.name}</h2>
              {action.is_system && (
            ...
            {action.description && <p className="text-sm text-muted-foreground mt-0.5">{action.description}</p>}
            {action.trigger_type === 'on_incident' && (
              <p className="text-xs text-muted-foreground mt-1">Triggered automatically when an incident is resolved from the Incidents page.</p>
            )}
          </div>
          <div className="flex items-center gap-3">
            ...
            {action.trigger_type !== 'on_incident' && (
              <button onClick={handleRunNow} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted border border-border/50 text-xs font-medium text-foreground hover:text-foreground hover:bg-muted/80 transition-all">
                <Play className="h-3 w-3" /> Run Now
              </button>
            )}
            <button onClick={onEdit} className="px-3 py-1.5 rounded-lg bg-muted border border-border/50 text-xs font-medium text-foreground hover:text-foreground hover:bg-muted/80 transition-all">
              Edit
            </button>
            {action.is_system && action.is_modified && (
              <button onClick={handleRestoreDefault} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted border border-indigo-500/30 text-xs font-medium text-indigo-400 hover:text-indigo-300 hover:bg-indigo-900/20 transition-all">
                <RotateCcw className="h-3 w-3" /> Restore Default
              </button>
            )}
            {!action.is_system && (
              <button onClick={handleDelete} className="px-3 py-1.5 rounded-lg bg-muted border border-red-900/30 text-xs font-medium text-red-400 hover:text-red-300 hover:bg-red-900/20 transition-all">
                Delete
              </button>
            )}
          </div>
      ...
      <Panel title="Configuration">
        <div className="space-y-4">
          ...
          <div>
            <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider mb-2">Instructions</p>
            <div className="bg-background/50 border border-border/50 rounded-lg p-3 text-sm text-foreground whitespace-pre-wrap leading-relaxed">
              {action.instructions}
            </div>
          </div>
          {action.mode === 'agent' && (
        ...
        {runs.length === 0 ? (
          ...
          <div className="overflow-hidden rounded-lg border border-border/60">
            <table className="w-full text-sm">
              ...
              <tbody>
                {runs.map((run) => (
                  <tr key={run.id} className={`border-b border-border/40 hover:bg-muted/20 transition-colors duration-150 ${run.status === 'error' ? 'bg-red-500/[0.03]' : ''}`}>
                    ...
                    <td className="px-4 py-2.5 text-xs text-muted-foreground" style={{ fontVariantNumeric: 'tabular-nums' }}>
                    ...
                    <td className="px-4 py-2.5">
                      {run.chat_session_id && (run.status === 'success' || run.status === 'error') && (
                        <a href={`/chat?sessionId=${run.chat_session_id}`} className="text-xs text-muted-foreground hover:text-foreground transition-colors">
                          View Chat
                        </a>
                      )}
                    </td>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h2 className="text-xl font-semibold tracking-tight text-foreground">{isEdit ? 'Edit Action' : 'Create Action'}</h2>
        <p className="text-sm text-muted-foreground mt-0.5">Define natural language instructions that Aurora executes as a background agent task.</p>
      </div>
          ...
          <Panel title="Details">
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="action-name" className="text-xs text-muted-foreground">Name</Label>
              ...
              <div className="space-y-2">
                <Label htmlFor="action-desc" className="text-xs text-muted-foreground">Description</Label>
        ...
        <div className="space-y-5">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 px-3 py-2 rounded-lg bg-primary text-primary-foreground text-xs font-medium hover:bg-primary/90 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
            >
              {submitting ? submittingLabel : submitLabel}
            </button>
            <button onClick={onBack} className="px-3 py-2 rounded-lg bg-muted border border-border/50 text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-muted/80 transition-all">
              Cancel
            </button>
          </div>
        </div>
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">One-click, governed operations — run a specialist agent on an incident. Reusable from incidents and workflow action nodes.</p>
        </div>
        {!createOpen && !selectedActionId && !editingActionId && (
          <button
            onClick={() => setCreateOpen(true)}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted border border-border/50 text-xs font-medium text-foreground hover:text-foreground hover:bg-muted/80 transition-all"
          >
...
function EditActionWrapper({ actionId, onBack, onSaved }: { readonly actionId: string; readonly onBack: () => void; readonly onSaved: () => void }) {
  ...
  if (!data?.action) return <div className="text-sm text-muted-foreground py-12 text-center">Loading...</div>;
